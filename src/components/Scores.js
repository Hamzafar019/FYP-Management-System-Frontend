...
function Scores() {
  ...
  return (
    <div>
      ...
      {!error && !!marksData.length && ( // Check if marksData is not empty before rendering
        <ul>
          {marksData.map((group, index) => (
            <li key={index} style={{ border: '1px solid #ccc', borderRadius: '8px', padding: '7px', marginBottom: '16px', marginTop: '16px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
